...
# Common Cryptographic Algorihms Revision D.1
...
cave_ibox = [
    
]
...
algorithms = {
    ...
    "enc_snow_alpha_mul":   (snow_alpha_mul,'L'),
    "enc_snow_alpha_invmul":(snow_alpha_invmul,'L'),
    "enc_zuc_S0":           (zuc_S0,'B'),
    "enc_zuc_S1":           (zuc_S1,'B'),
    "enc_zuc_const":        (zuc_const,'H'),
    "enc_rijndael_T0":      (rijndael_T0,'L'),
    "enc_rijndael_T1":      (rijndael_T1,'L'),
    "enc_rijndael_T2":      (rijndael_T2,'L'),
    "enc_rijndael_T3":      (rijndael_T3,'L'),
}
...
class Trie(dict):
    ...
    def add_keyword(self,keyword,ret):
        if 0 != len(keyword)%self.chunk_size:
            raise Exception("keyword not a multiple of chunk_size")

        last_trie = self

        for x in xrange(0,len(keyword),self.chunk_size):
            trie_key = keyword[x:x+self.chunk_size]

            if trie_key not in last_trie:
                last_trie[trie_key] = Trie()

            last_trie = last_trie[trie_key]
               
        last_trie.ret = ret
            
    def search_string(self,s,buf_size = None,align=2):
        match_buffer = []
        idx = 0
        buf = None
        while idx < len(s):
            ...
            if buf_size is not None:
                tmp_idx = idx % (buf_size/2)
                if tmp_idx == 0:
                    buf = s[idx:idx+buf_size]
                c = buf[tmp_idx:tmp_idx+self.chunk_size]
            else:
                c = s[idx:idx+self.chunk_size]

            for i in range(len(match_buffer)-1,-1,-1):
                if idx == match_buffer[i][1] + self.chunk_size*match_buffer[i][2]:
                    if c in match_buffer[i][0].keys():
                        match_buffer[i][2] += 1
                        if match_buffer[i][0][c].ret is not None:
                            yield ( match_buffer[i][0][c].ret,
                                    match_buffer[i][1],
                                    match_buffer[i][2]*self.chunk_size)

                        match_buffer[i][0] = match_buffer[i][0][c]
                    else:
                        match_buffer.pop(i)
    ...
    def match(self,s,idx):
        if self.ret is not None:
            yield self.ret
        c = s[idx]
        if c in self: 
            for result in self[c].match(s,idx+1):
                yield result
            

def findDatas(s,algos,chunk_size=16):
    units = ['B',"H",'I','L','Q']
    endian = ['<','>']
    chunk_level = 1
    ...
    start_trie = Trie(chunk_size=chunk_size)
    ...
    for name in algos:
        ...
        for unit in units[units.index(min_unit):]:
            for bom in endian:
                packing = bom+''.join(unit for _tmp in range(len(dataset)))
                start_chunk = struct.pack(packing,*dataset)
                start_trie.add_keyword(start_chunk,(name,bom,unit,len(dataset),0))
    for ret in start_trie.search_string(s):
        yield ret
...
if __name__ == '__main__':
    ...
    for fn in sys.argv[1:]: 
        fh = open(fn)
        fh_mmap = mmap.mmap(fh.fileno(), 0, access=mmap.ACCESS_READ)

        for result in findDatas(fh_mmap, algorithms, chunk_size=32):
            print result
            found = True
